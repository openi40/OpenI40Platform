...
export class Openi40HomepageComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.apsDataListService.getListDataSetEntry().subscribe((entries)=>{
      this.dataSetEntries=entries;
      if (this.dataSetEntries && this.dataSetEntries.length===1) {
        this.onChoosedDataSource(this.dataSetEntries[0].dataSourceName,this.dataSetEntries[0].dataSetName,this.dataSetEntries[0].dataSetVariant);
      }
    })
  }
