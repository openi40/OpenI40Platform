/**
 * 
 * This code is part of the OpenI40 open source advanced production scheduler
 * platform suite, have look to its licencing options.
 * Web site: http://openi40.org/  
 * Github: https://github.com/openi40/OpenI40Platform
 * We hope you enjoy implementing new amazing projects with it.
 * @author architectures@openi40.org
 *
 */
package com.openi40.platform.persistence.input.channel.providers;

import org.springframework.stereotype.Repository;

import com.openi40.platform.persistence.input.channel.model.AcquiredSetupSecondaryResourceInfo;
import com.openi40.scheduler.input.model.tasks.UsedSecondaryResourcesInfoInputDto;
@Repository
public interface AcquiredSetupSecondaryResourceInfoRepository extends InputChannelRepository<AcquiredSetupSecondaryResourceInfo,UsedSecondaryResourcesInfoInputDto> {

}
