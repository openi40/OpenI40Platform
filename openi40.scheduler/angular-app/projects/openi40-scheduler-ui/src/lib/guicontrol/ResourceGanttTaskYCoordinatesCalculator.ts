...
export class ResourceGanttTaskYCoordinatesCalculator implements ITaskYCoordinatesCalculator {
  ...
  getReferenceBlock(_task: TaskGuiItem, retrieverMap: YBlocksRetrieverMap): BaseGuiItem {
    let resource = _task.data.scheduledMachineCode;
    if (resource) {
      return retrieverMap.get("MachineGuiItem", resource);
    }
    else
      return null;
  }
