	...
	public List<DateRule> rebuildRules(List<DateRule> actualRules, Task task, ApsLogicOptions options) {
		...
		if (task.getAskedDeliveryDateTime() != null) {
			...
			dateConstraintRule.setUnmetConstraintMessage(
					new ApsMessage(this, ApsMessageConstrants.DELIVERY_DATE_CONSTRAINT_NOT_MET, environment,task.getContext()));
		...
		windowMinimumBound.setUnmetConstraintMessage(
				new ApsMessage(this, ApsMessageConstrants.WINDOW_DATE_START_DATE_CONSTRAINT_NOT_MET, environment,task.getContext()));
		windowMaximumBound.setUnmetConstraintMessage(
				new ApsMessage(this, ApsMessageConstrants.WINDOW_DATE_END_DATE_CONSTRAINT_NOT_MET, environment,task.getContext()));
		...
		if (task.getMaxProductionDateConstraint() != null) {
			...
			MaximumProductionBound.setUnmetConstraintMessage(
					new ApsMessage(this, ApsMessageConstrants.MAX_PRODUCTION_END_DATE_CONSTRAINT_NOT_MET, environment,task.getContext()));
		}
		return dateRules;
	}
