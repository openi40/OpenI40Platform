...
public class TasksRelationRulesGeneratorImpl extends BusinessLogic<Task> implements ITasksRelationRulesGenerator {
	...
	public List<TasksRelationRule> rebuildRules(List<TasksRelationRule> actualRules, Task task,
		...
		IApsLogic algorithm = componentsFactory.create(IApsLogic.class,
				task.getParentSchedulingSet().getAlgorithmType(), task.getParentSchedulingSet(), task.getContext());
		...
		if (algorithm.getDirection() == ApsLogicDirection.FORWARD) {
			...
			for (TaskEdge edge : childTasks) {
				...
				TasksRelationRule finishAfterChildConstraintRule = new TasksRelationRule(task,
						Rule.ConstraintOrigin.SCHEDULING, Rule.ConstraintPriority.WARNS, childTask, edge);

				finishAfterChildConstraintRule.setUnmetConstraintMessage(new ApsMessage(this, task,
						ApsMessageConstrants.FINISH_AFTER_CHILD_CONSTRAINT_RULE, environment, task.getContext()));
			...
			if (task.getParentTask() != null) {
				...
				TasksRelationRule finishAtParentStart = new TasksRelationRule(task, Rule.ConstraintOrigin.SCHEDULING,
						Rule.ConstraintPriority.WARNS, task.getParentTask().getConsumerTask(), task.getParentTask());
				finishAtParentStart.setUnmetConstraintMessage(new ApsMessage(this, task,
						ApsMessageConstrants.FINISH_TASK_TIME_FOLLOWING_TASK_START_TIME_RELATION, environment,
						task.getContext()));
